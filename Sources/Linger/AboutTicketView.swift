...
final class AboutTicketView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        // 点状纹理（双层，原型 radial-gradient 双层点）
        let dotColor = NSColor(calibratedWhite: 0, alpha: 0.015)
        dotColor.setFill()
        for yy in stride(from: 1, to: Int(bounds.height), by: 3) {
            for xx in stride(from: 1, to: Int(bounds.width), by: 3) {
                NSBezierPath(ovalIn: NSRect(x: CGFloat(xx), y: CGFloat(yy), width: 1, height: 1)).fill()
            }
        }
        let dot2 = NSColor(calibratedWhite: 0, alpha: 0.010)
        dot2.setFill()
        for yy in stride(from: 2, to: Int(bounds.height), by: 7) {
            for xx in stride(from: 2, to: Int(bounds.width), by: 7) {
                NSBezierPath(ovalIn: NSRect(x: CGFloat(xx), y: CGFloat(yy), width: 1, height: 1)).fill()
            }
        }
